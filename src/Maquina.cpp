...
void Maquina::dibuja()
{
	...
	case JUEGO:
		...
		break;
	case JUGANDO:
		tab.dibuja(Maquina::fil, Maquina::col); //tab.dibuja(mund::fil,mundo::col) //SELECCION DE CASILLA, FALTA LA CONVERSION DE EJE A CASILLA
		break;
	}
}
...
///*
void Maquina::raton(int button, int state, int x, int y)
{
	switch (estado)
	{
	case PANT_INI:
	...
	case JUEGO:
		if ((button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) && (x > 235) && (x < 565) && (y > 135) && (y < 465))
		{
			...
			Maquina::col = (x - 330) / 41.25;
			Maquina::fil = (y - 330) / 41.25;
			std::cout << "casilla x" << Maquina::fil << "casilla y " << Maquina::col;
		}
	case JUGANDO:
		if ((button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) && (x > 235) && (x < 565) && (y > 135) && (y < 465))
		{
			...
			estado = JUGANDO;
			//conversion de x y a fil col
			Maquina::col = (x - 235) / 41.25;
			Maquina::fil = (y - 135) / 41.25;
			std::cout << "casilla x" << Maquina::fil << "casilla y " << Maquina::col;
		}
	}
}
